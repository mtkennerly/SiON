...
import org.si.sound.patterns.PMLParser;

/** Preset patterns for DrumMachine */
class DrumMachinePresetPattern
{
    // variables
    //----------------------------------------
    /** categoly list. */
    ...
    private var _pp : PMLParser = new PMLParser({
            0 : new Note(-1, -1, Float.NaN, 0),
            1 : new Note(-1, -1, Float.NaN, 1),

        });
    private function _pattern(key : String, pml : String) : Void{
        var pattern : Array<Note> = _pp.parse(pml);
